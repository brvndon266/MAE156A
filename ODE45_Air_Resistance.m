%% Pendulum Estimate Swing Time with Air Resistance using Runge-Kutta Method
...
b = 0.0005;   % damping coefficient due to air resistance (adjust this as needed)
...
% Function to calculate angular acceleration (alpha) with air resistance
alpha_func = @(theta, omega, r) (-m * g * L * sin(theta) - mu * sign(omega) * m * g * r - b * omega) / (m * L^2);
...
for i = 1:length(radii)
    ...
    title(['Pendulum Swing (Radius = ', num2str(r * 39.37), ' inches) with Air Resistance']);
    xlabel('Time (s)');
