...
for i = 1:length(radii)
    ...
    for n = 1:length(t)-1
        ...
        if r == 0.125 * 0.0254  % for 1/8 inch radius
            friction_factor = 1.0;  % no additional friction adjustment
        elseif r == 0.25 * 0.0254  % for 1/4 inch radius
            friction_factor = 1.2;  % slight adjustment
        else
    ...
    title(['Pendulum Swing (Radius = ', num2str(r * 39.37), ' inches) small angle & \mu = 0 (Actual Pendulum)']);
    xlabel('Time (s)');
